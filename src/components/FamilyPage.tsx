...
const AVATAR_COLORS = [
  '#E96D1F', '#3B82F6', '#8B5CF6', '#EC4899', '#10B981', '#F59E0B'
]

const RELATIONSHIPS: Relationship[] = ['Mum', 'Dad', 'Brother', 'Sister', 'Partner', 'Other']
    ...
    <div className="flex min-h-screen flex-col items-center justify-center bg-[#FFFEEC] px-4">
      ...
      <div className="relative z-10 flex flex-col items-center text-center">
        <a href="/" aria-label="Subsecute home">
          <img src="/images/landing/logo.png" alt="Subsecute logo" className="mb-8 h-8 w-auto" />
        </a>
  ...
  return (
    <div className="flex min-h-screen flex-col bg-[#FFFEEC]">
      <div className="mx-auto flex w-full max-w-[560px] flex-1 flex-col px-4 pb-8 pt-6">
        {/* Header */}
        <a href="/" aria-label="Subsecute home" className="mb-6 self-start">
          <img src="/images/landing/logo.png" alt="Subsecute logo" className="h-7 w-auto" />
        </a>
            ...
            {members.map((m, i) => (
              ...
              >
                ...
                <div className="flex-1">
                  <p className="font-outfit text-sm font-medium text-[#232323]">{m.name}</p>
                  <p className="font-outfit text-xs text-[#6C757D]">{m.relationship}</p>
                </div>
                <button
  ...
  return (
    <div className="flex min-h-screen flex-col bg-[#FFFEEC]">
      <div className="mx-auto flex w-full max-w-[560px] flex-1 flex-col px-4 pb-8 pt-6">
        ...
        <p className="mt-1 font-outfit text-sm text-[#6C757D]">
          Select the bills and tiers you cover for {member.name} ({member.relationship}).
        </p>

        {/* Bill grid */}
        <div className="mt-5 flex flex-col gap-4">
          {BILL_OPTIONS.map((bill) => (
            <div key={bill.label} className="rounded-xl border border-[#E5E7EB] bg-white p-4">
              <p className="mb-2.5 font-outfit text-sm font-semibold text-[#232323]">{bill.label}</p>
              <div className="flex flex-wrap gap-2">
                {bill.tiers.map((tier) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="block text-[11px]">{formatNaira(tier.amount)}/mo</span>
                    </button>
                  )
                })}
              </div>
          ...
          <div className="rounded-xl border border-[#E5E7EB] bg-white p-4">
            <p className="mb-2.5 font-outfit text-sm font-semibold text-[#232323]">Custom</p>
      ...
      <div className="mx-auto flex w-full max-w-[560px] flex-1 flex-col px-4 pb-8 pt-6">
        ...
        <div className="mt-5 rounded-2xl border border-[#E96D1F]/20 bg-gradient-to-br from-[#E96D1F]/5 to-[#E96D1F]/10 p-5 text-center">
          ...
          <p className="mt-1 font-neue-power text-4xl font-bold text-[#E96D1F] sm:text-5xl">
            {formatUSD(totalAnnual)}<span className="text-2xl sm:text-3xl">/year</span>
          </p>
        ...
        <div className="mt-4 grid grid-cols-2 gap-3">
          <div className="rounded-xl border border-[#E5E7EB] bg-white p-4">
            <p className="font-outfit text-[11px] font-medium uppercase tracking-wider text-[#ADB5BD]">Monthly</p>
            <p className="mt-1 font-neue-power text-lg font-bold text-[#232323]">{formatNaira(totalMonthly)}</p>
            <p className="font-outfit text-xs text-[#6C757D]">{formatUSD(totalMonthly)}</p>
          </div>
          <div className="rounded-xl border border-[#E5E7EB] bg-white p-4">
            <p className="font-outfit text-[11px] font-medium uppercase tracking-wider text-[#ADB5BD]">Annual</p>
            <p className="mt-1 font-neue-power text-lg font-bold text-[#232323]">{formatNaira(totalAnnual)}</p>
            <p className="font-outfit text-xs text-[#6C757D]">{formatUSD(totalAnnual)}</p>
          </div>
        </div>
        ...
        <div className="mt-3 flex flex-col gap-3">
          {memberTotals.map(({ member, index, monthly, bills }) => (
            <div key={member.id} className="rounded-xl border border-[#E5E7EB] bg-white p-4">
              <div className="flex items-center gap-3">
                <Avatar name={member.name} index={index} />
                <div className="flex-1">
                  <p className="font-outfit text-sm font-medium text-[#232323]">{member.name}</p>
                  <p className="font-outfit text-xs text-[#6C757D]">{member.relationship}</p>
                </div>
                <div className="text-right">
                  <p className="font-outfit text-sm font-semibold text-[#232323]">{formatNaira(monthly)}/mo</p>
                  <p className="font-outfit text-[11px] text-[#6C757D]">{formatUSD(monthly)}/mo</p>
                </div>
              </div>
              {/* Bill tags */}
              {(Object.keys(bills.bills).length > 0 || bills.custom.length > 0) && (
          ...
          {status === 'success' ? (
            <div className="flex flex-col items-center gap-4 text-center">
              ...
              <p className="font-outfit text-sm font-medium text-[#232323]">
                You&apos;re on the list! We&apos;ll notify you when Subsecute can automate all of this.
              </p>
            ...
            <>
              <p className="text-center font-outfit text-sm font-medium text-[#232323]">
                Save this family payment plan and get notified when Subsecute can automate all of it.
              </p>
              ...
              {status === 'error' && (
                <p className="mt-2 text-center font-outfit text-xs text-red-500">{errorMsg}</p>
              )}
            </>
          )}
        </div>
...
export default function FamilyPage() {
  ...
  return (
      <main className="font-neue-power">
        {step === 'landing' && <LandingScreen onStart={() => setStep('members')} />}
        {step === 'members' && (
          <AddMembersStep
            members={members}
